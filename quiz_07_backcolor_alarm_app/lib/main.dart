...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String currentDateTimeMin = "";
  String chosenDateTimeStringMin = "";
  ...
  Widget build(BuildContext context) {
    return CupertinoApp(
        ...
        home: CupertinoPageScaffold(
            backgroundColor: currentDateTimeMin == chosenDateTimeStringMin
                ? now.second % 2 == 0
                    ? Colors.amber
                    : Colors.blueAccent
                : Colors.white,
  ...
  void _addItem() {
    ...
    setState(() {
      currentDateTimeMin =
          "${now.year}-${zero.monthZero(now.month)}-${zero.dayZero(now.day)} ${zero.weekday(now.weekday)} ${zero.hourZero(now.hour)}:${zero.minuteZero(now.minute)}";
      currentDateTime =
  ...
  String _chosenItem(DateTime now) {
    ...
    setState(() {
      chosenDateTimeStringMin =
          "${now.year}-${zero.monthZero(now.month)}-${zero.dayZero(now.day)} ${zero.weekday(now.weekday)} ${zero.hourZero(now.hour)}:${zero.minuteZero(now.minute)}";

      chosenDateTimeString =
          "${now.year}-${zero.monthZero(now.month)}-${zero.dayZero(now.day)} ${zero.weekday(now.weekday)} ${zero.hourZero(now.hour)}:${zero.minuteZero(now.minute)}:${zero.secondZero(now.second)}";
    });
    return chosenDateTimeString;
  }
}
